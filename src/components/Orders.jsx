    ...
    const showOrderDetails = async (orderId) => {
        try {
            ...
            `;
                ...
                const selectedProduct = orderDetails[0].data();
                                        ...
                                        <td className='text-center'>
                                            <h6 style={{ whiteSpace: 'nowrap', width: '10rem', overflow: 'hidden', textOverFlow: 'ellipsis', fontSize: '18px' }}>
                                                {item.status === 'Pendiente' ? (
                                                 <button type="button" className="btn btn-warning p-1" onClick={() => openShippingStatus(item)}>{item.status}</button>   
                                                ) : (
